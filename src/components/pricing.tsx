...
export const PricingBox = ({ title, year, month, lifetime, benefits }: { title: string, year: number, month: number, lifetime: number, benefits: string[] }) => {
    ...
    let glowColor = glowColors.purple;
    if (title == "Free") {
        glowColor = glowColors.rose;
    } else if (title == "Premium") {
        glowColor = glowColors.violet;
    } else if (title == "Student") {
        glowColor = glowColors.blue;
    }

    return (
        <div className={`border border-gray-800 hover:border-purple-600 m-4 h-full flex flex-col p-8 rounded-3xl bg-gradient-to-br from-gray-900 to-black transition-all duration-300 ${glowColor}`}>
